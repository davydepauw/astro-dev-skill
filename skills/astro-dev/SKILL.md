...
**3. `render()` is a standalone function:**
```ts
// agents generate this (outdated)
...
**4. Integration plugins run before your remarkPlugins:**
...
**5. Choose the right `client:` directive — both directions matter:**
```astro
...
**6. Use Actions for forms, not manual API routes:**
```ts
...
**7. Cookies, sessions, and forms require on-demand rendering:**
```astro
---
...
**8. Use `astro:env` for environment variables, not `process.env`:**
```ts
...
**9. Styles are scoped — `class` doesn't pass through to children:**
```astro
...
**10. `<script>` is deduplicated — don't expect per-instance behavior:**
```astro
...
**11. `fetch()` in frontmatter runs at build time, not per request:**
```astro
---
...
**12. Don't build manual locale routing — use Astro's built-in i18n:**
```ts
// astro.config.ts
...
**13. Import Zod from `astro/zod`, not from `astro:content`:**
```ts
...
**14. Legacy content collections are fully removed in Astro 6:**
```ts
// ERRORS in Astro 6:
...
**15. CJS config files are no longer supported:**
```ts
// ERRORS in Astro 6
...
**16. With `<ClientRouter />`: use `astro:page-load`, not direct calls:**
```ts
...
**17. With `<ClientRouter />`: use event delegation, not direct listeners:**
```ts
...
**18. Preserve theme/state in `astro:before-swap`, not `after-swap`:**
```ts
...
**19. Visibility CSS (`display: none`) must be in global.css, not component styles:**
